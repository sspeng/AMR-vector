...
void FEMParameters::read(GetPot &input)
{
  ...
  GETPOT_INPUT(ramp_parameter);
  GETPOT_INPUT(initial_density);
  GETPOT_INPUT(minimum_density);
  GETPOT_INPUT(filter_parameter);
  GETPOT_INT_INPUT(pnorm_parameter);
